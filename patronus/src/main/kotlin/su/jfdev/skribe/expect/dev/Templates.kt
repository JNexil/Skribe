...
inline fun <T: Any> Backend<T>.negation(text: String, assert: Backend<T>.() -> Boolean): Expect<T> = append(text).negate(assert)
inline fun <T: Any> Backend<T>.assertion(text: String, assert: Backend<T>.() -> Boolean): Expect<T> = append(text).assert(assert)